...
# PDU
계층에서 계층으로 데이터 전달 시 한 덩어리의 단위.

|계층|PDU|
|애플리케이션|메시지|
|전송(+TCP 헤더)|세그먼트(TCP), 데이터그램(UDP)|
|인터넷(+IP 헤더)|패킷|
|링크(+프레임 헤더, 프레임 트레일러)|프레임|

# 네트워크 기기의 계층별 처리 범위
상위 계층을 처리하는 기기는 하위 계층을 처리 가능(반대로는 불가능)
...
서버의 부하를 분산함. 클라이언트로부터 오는 요청들을 여러 서버로 나눔. 시스템의 처리 가능 트래픽 증가

URL, 서버, 캐시, 쿠키 기반으로 트래픽 분산.

필터링 기능, 응용 프로그램 수준의 트래픽 모니터링 가능.

## 전송 계층 - L4 스위치
스트리밍 관련 서비스에서 사용 불가능, IP와 포트 기반으로 트래픽 분산
```
클라우드 서비스에서 L7 로드밸런싱은 ALB 컴포넌트, L4 로드밸런싱은 NLB 컴포넌트로 함.
```

### 헬스체크
L7 스위치와 L4 스위치 모두 헬스체크를 함.

전송 주기와 재전송 횟수 등을 설정해 반복적으로 서버에 요청을 보내는 것. 이를 통해 서버의 정상/비정상을 판별함.

### 로드밸런서 - 서버 이중화
서비스를 안정적으로 운영하기 위해서는 2대 이상의 서버가 필수적임. 로드밸런서는 2대 이상의 서버를 기반으로 가상 IP를 제공하고 이를 기반으로 안정적으로 서비스 제공.


## 인터넷 계층 - 라우터
여러 개의 네트워크를 연결, 분할, 구분하는 역할.

패킷 소모를 최소화하고 경로를 최적화하여 최소 경로로 패킷을 포워딩함.
## 인터넷 계층 - L3 스위치
L2스위치의 기능과 라우팅 기능을 갖춘 장비. 하드웨어 기반의 라우팅 담당.
## 데이터 링크 계층 - L2 스위치
장치들의 MAC주소를 MAC 주소 테이블을 통해 관리. 연결된 장치로부터 패킷이 오면 패킷을 전송함.(단순 스위칭)
## 데이터 링크 계층 - 브리지
두 개의 근거리 통신망의 상호 접속을 하게 하는 통신망 연결 장치.

통신망 범위 확장. 서로 다른 LAN 등으로 이루어진 하나의 통신망 구축 시 사용.
## 물리 계층 - NIC
LAN카드. 2대 이상의 컴퓨터 네트워크 구성에 사용됨. MAC주소가 있음.
## 물리 계층 - 리피터
들어오는 약해진 신호 정도를 증폭하여 다른 쪽으로 전달하는 장치.(이제 거의 사용X)
## AP
패킷을 복사하는 기기.
